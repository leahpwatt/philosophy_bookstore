...
class BookItem extends React.Component{
  ...
  handleCart(){
    ...
    if(this.props.cart.length > 0){
      //IF CART IS NOT EMPTY, CHECK IF THERE IS PRODUCT WITH SAME ID
      let _id = this.props._id;

      let cartIndex = this.props.cart.findIndex(function(cart){
        return cart._id === _id;
      })
      // If RETURNS -1 THERE ARE NO ITEMS WITH SAME ID SO ADD PRODUCT
      if (cartIndex === -1){
        this.props.addToCart(book);
      } else {
        //ONLY NEED TO UPDATE QUANTITY; DISPATCH ACTION
